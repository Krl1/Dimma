...
print(cec_images[START_IDX:START_IDX+TRAIN_SIZE])

def load_pair(idx):
    light = cv2.imread(str(cec_images[idx]))[..., ::-1]
    dark = cv2.imread(f'{CEC_PATH}/train/corrupted/{cec_images[idx].name}')[..., ::-1]
...
def test_model(model):
    # read image
    test_clean = list(sorted(Path(f'{CEC_PATH}/test/clean/').glob('*.png'), key=lambda x: int(x.stem)))
    l = cv2.imread(str(test_clean[0]))[..., ::-1] / 255.0  # pierwszy obraz testowy
    d = cv2.imread(f'{CEC_PATH}/test/corrupted/{test_clean[0].name}')[..., ::-1] / 255.0
